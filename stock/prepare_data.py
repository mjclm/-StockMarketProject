...
# TODO: modify the function to allow setting the shift


def features_creation_rollings(df, id_col, date_col, target, windows, func=None):
    if func is None:
        func = ['mean']
    df_copy = df.copy().sort_values([id_col, date_col])
    df_copy = df_copy.assign(**{
        'rolling_{}_{}'.format(f, w): df_copy.groupby(id_col)[col]
        .transform(lambda x: x.shift(1).rolling(w).agg(f))
        for w in windows for f in func for col in target})
    return df_copy


def features_creation_rollings_with_sliding(df, id_col, date_col, target, windows, shifts, func=['mean']):
    df_copy = df.copy().sort_values([id_col, date_col])
    df_copy = df_copy.assign(**{
...
def features_creation_date_extract_components(df, date_col):
    # TODO: implement a function with .__getattribute__(**dict(<keys:elements>))
    # return df[date_col].apply(lambda x: {attr: x.__getattribute__(attr) for attr in attributes}).apply(pd.Series)
    df_copy = df.copy()
    df_copy[['month', 'dayofyear', 'quarter', 'dayofweek', 'days_in_month', 'weekofyear', ]] = df_copy[date_col].apply(lambda x:
